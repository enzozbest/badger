...
class TestPaymentStatus(TestCase):
    def setUp(self):
        ...
        self.invoice_id = generate_invoice_id(self.request_alloc.student, get_latest_id_number(self.request_alloc.student))
    ...
    def generate_invoice(self) -> HttpResponse:
        self.client.force_login(self.admin)
        return self.client.get(f'{reverse("generate_invoice", kwargs={"request_id": self.request_alloc.id})}')
