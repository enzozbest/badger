...
class MakeUserAdminTestCase(TestCase):
    ...
    def test_unauthenticated_user_make_user_admin(self):
        ...
        self.assertRedirects(response, f"{reverse('log_in')}?next={reverse('make_admin', args=[2])}", status_code=302,
    ...
    def test_unauthenticated_user_confirm_make_user_admin(self):
        ...
        self.assertRedirects(response, f"{reverse('log_in')}?next={reverse('confirm_make_admin', args=[2])}",
                             status_code=302, target_status_code=200)
        self.assertTemplateUsed(response, 'log_in.html')

    def test_nonadmin_user_make_user_admin(self):
        self.user = User.objects.create_user(username='testuser2', email='test2@test.com', password='Password123',
                                             user_type='Tutor')
